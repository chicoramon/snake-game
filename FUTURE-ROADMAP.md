...
- **Supabase player and leaderboard services — implemented locally (2026-07-28).** Safe public-client creation, anonymous/session bootstrap transport, email-code transport, player-profile reads and writes, score submission, mode-wide record lookup, standard and Daily leaderboard pages, historical Daily archive statistics, and public player-card requests now live behind independently tested service modules. `main.js` retains the visible identity, retry, leaderboard, and dialog orchestration that coordinates those services.
- **Player-facing UI orchestration and composition boundary — implemented locally (2026-07-28).** Theme selection, random/theme-mode wiring, Daily Run rules, What’s New, and Player-panel input/event binding now live in dedicated UI modules. `main.js` composes those views with explicit stateful callbacks and separately composed game, audio, controls, renderer, lifecycle, Daily Run, player, and leaderboard services; the remaining entry-file code is the game-specific state and presentation callbacks that connect those modules.
